...
const orderCart = (cart = null) => {
	...
	let _cart = cart.map(item => {
		// let novaAltura = Math.min( item.alturaCm, item.larguraCm );
		let novoComprimento = Math.max( item.alturaCm, item.larguraCm );//Comprimento == maior dimensão horizontal
        //let _temp = [item.alturaCm, item.larguraCm].sort((a,b) => a < b);
		item.larguraCm = item.larguraCm;
		item.profundidadeCm = novoComprimento;
		item.alturaCm = item.alturaCm;
